...
interface Message {
  ...
  text: string;
  ts: string;
  files?: { thumb_720: string }[];
}

export function filterMessages(originalData: OriginalData): Message[] {
  // Access the nested messages array
  const nestedMessages = originalData.messages[0].messages;

  return nestedMessages.map((msg: NestedMessage) => {
    // Prepare the files array if it exists, focusing on the thumb_720 property
    const files =
      msg.files
        ?.filter((file: File) => file.thumb_720 !== undefined) // Ensure thumb_720 is not undefined
        .map((file: File) => ({
          thumb_720: file.thumb_720 as string, // Type assertion to satisfy the compiler
        })) || [];
